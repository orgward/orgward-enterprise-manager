...
test('projects remain isolated and static UI exposes the map workflow', async (t) => {
  ...
  assert.match(html, /id="map-search" type="search" maxlength="120"/);
  assert.match(html, /id="map-search-status" role="status" aria-live="polite"/);
  assert.match(html, /id="founder-announcement" class="sr-only" role="status" aria-live="polite" aria-atomic="true"/);
  assert.match(html, /id="map-results-jump" class="button ghost" type="button" aria-describedby="map-search-status"/);
  assert.match(html, /id="execution-nav" href="\/execution\.html"/);
  ...
  assert.match(script, /process-dependency-select/);
  assert.match(script, /label\.toLowerCase\(\)/);
  assert.match(script, /do not allocate resources, operate systems, or grant authority/);
  assert.match(script, /resourceIds/);
  assert.match(script, /systemIds/);
  assert.match(script, /capabilityId/);
  assert.match(script, /does not assign work, activate a capability, or grant authority/);
  assert.match(script, /decisionMakerRoleId/);
  assert.match(script, /decisionScopeIds/);
  assert.match(script, /not an endorsement or approval/);
  assert.match(script, /agent actions, or execution authority/);
  assert.match(script, /encodeExecutionRoute/);
  assert.match(script, /syncExecutionNavigation/);
  assert.match(script, /strategyGoalIds/);
  assert.match(script, /design intent, not achievement/);
  assert.match(script, /inputDecisionIds/);
  assert.match(script, /outputDecisionIds/);
  assert.match(script, /Decision inputs/);
  assert.match(script, /Decision outputs/);
  assert.match(script, /does not approve decisions or authorize execution/);
  assert.match(script, /does not approve a decision, trigger execution, or change permissions/);
  assert.match(script, /Decision outputs are edited separately/);
  assert.match(script, /feedbackGoalId/);
  assert.match(script, /Proposed steering goal/);
  assert.match(script, /not evidence of goal achievement or approval/);
  assert.match(script, /Decision inputs and outputs are edited separately/);
  assert.match(script, /feedbackDecisionIds/);
  assert.match(script, /searchGraph\(areaFiltered, state\.mapSearch\)/);
  assert.match(script, /Decisions guiding this feedback loop \(proposed\)/);
  assert.match(script, /Describes the intended design relationship; it does not approve a decision or authorize work/);
  assert.match(styles, /\.feedback-decision-select/);
  assert.match(styles, /\.feedback-decision-option/);
  assert.match(styles, /\.decision-scope-select/);
  assert.match(styles, /\.decision-scope-option/);
  assert.match(styles, /\.strategy-goal-select/);
  assert.match(styles, /\.strategy-goal-option/);
  assert.match(styles, /\.process-decision-select/);
  assert.match(styles, /\.process-decision-option/);
});
